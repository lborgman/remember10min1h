...
console.log("here is test-jsmind.js");
...
// debugger;
if (hasTouchEvents()) theMirrorWay = "pointHandle";
theMirrorWay = "pointHandle"; // FIX-ME:

switchMirrorWay(theMirrorWay);
checkTheMirrorWay();
...
function checkPointHandleDistance(evt) {
    if (posPointHandle.startX == undefined) {
        ...
        // if (pointPointHandle.dTop)
        // posPointHandle.nodeWidth = bcrT.width;
        // posPointHandle.nodeHeight = bcrT.height;
    ...
    // if (diff2 < diffPointHandle * diffPointHandle) return;
    const diffOk = !(diff2 < diffPointHandle * diffPointHandle);
    if (!diffOk) return;

    const dOutside = 10;
    // const nodeW = posPointHandle.nodeWidth;
    // const nodeH = posPointHandle.nodeHeight;
...
function applyShapeEtc(shapeEtc, eltJmnode) {
    // const shapeEtc = node.data.shapeEtc;
    // if (!shapeEtc) return;
    let eltShape = eltJmnode.querySelector(".jmnode-bg");
    if (!eltShape) {
        if (eltJmnode.childElementCount > 1) {
            console.error("old custom format 2");
            debugger;
            // return;
            let htmlRendererImg = mkElt("div", { class: "fc4i-image" });
...
async function pageSetup() {
    ...
    */
    ...
    function compareSizesDisplayedMirrored() {
        divMirroredWrapper = divMirroredWrapper || document.getElementById(idMirroredWrapper);
        if (!divMirroredWrapper) return;
    ...
    if (create) {
        ...
        mind = await DBgetMindmap(mindmapKey);
    }
    // debugger;
    if (!mind) {
        return;
    ...
    const eltJmnodesDisplayed =
        document.getElementById(optionsJmDisplay.container).querySelector("jmnodes");
    console.log("eltJmnodesDisplayed", eltJmnodesDisplayed);
    ...
    fixProblemsAndUpdateCustomAndShapes(jmDisplayed, eltJmnodesDisplayed);
    ...
    function finnishAndMirrorOperationOnNode(operation_type, operation_node_id, datadata) {
        ...
        switch (operation_type) {
            ...
            case "update_node":
                ...
                if (isCustomNode || !!fc4i_updated) {
                    theCustomRenderer.updateJmnodeFromCustom(jsMind.my_get_DOM_element_from_node(updated_node));
                    if (jmMirrored) {
            ...
            case "move_node":
                ...
                const moved_node = jmDisplayed.get_node(id_moved);
                const jmnodeMoved = jsMind.my_get_DOM_element_from_node(moved_node);
                fixJmnodeProblem(jmnodeMoved);
                ...
                if (customNeedUpdate) {
                    theCustomRenderer.updateJmnodeFromCustom(jsMind.my_get_DOM_element_from_node(moved_node));
                }
    ...
    */
    ...
    jsMindContainer.addEventListener("click", evt => {
        ...
        if (isOutsideO) {
            ...
            // const node = ourCustomRenderer.THEjmDisplayed.get_node(node_id);
            // console.log("*** old node.expanded, not saving", node.expanded, node);
            jmDisplayed.toggle_node(node_id);
            eltJmnode.classList.toggle("is-expanded");
            // console.log("*** new node.expanded, not saving", node.expanded, node);
            // return
    ...
    async function mkContextMenu() {
        ...
        async function pasteCustom2node() {
            ...
            if (!objCustom) return;
            convertDOMnodeToCopied4Mindmap(eltJmnode, jmDisplayed, objCustom);
        }

        const liTestConvertToCustom = mkMenuItem("Paste custom content into node", pasteCustom2node);
        ...
        const liTestMirror = mkMenuItem("Drag accessiblity", dialogDragAccessibility);
        ...
        async function OLDstartMirror(clientX4mirror, clientY4mirror) {
            ...
            function followMouse4mirror(evt) {
                mirrorVals.clientXthrottle4mirror = evt.clientX;
                mirrorVals.clientYthrottle4mirror = evt.clientY;
    ...
    */
    ...
    function convertDOMnodeToCopied4Mindmap(eltJmnode, jmOwner, objCustomCopied) {
        ...
        if (lastElementChild) {
            if (lastElementChild.classList.contains("jsmind-custom")) {
                alert("Already fc4i, not handled yet");
                return;
            }
        }
        ...
        if (provider != "fc4i") throw Error(`provider is not fc4i: ${provider}`);
        ...
        if (jmOwner) {
            ...
            // fixNewFc4iNode(node_id); // FIX-ME:
            addJmnodeBgAndText(eltJmnode);
            const eltRendererImg = eltJmnode.lastElementChild;
            fixRenderImg(eltRendererImg);
            // ourCustomRenderer.updateJmnodeFromCustom(eltJmnode, jmOwner);
        } else {
            // debugger; // FIX-ME: ???
    ...
    async function OLDfixNewFc4iNode(node_id) {
        debugger; // Should not be used any more!
        ...
        eltFc4iLink.classList.add("jsmind-custom-image");
        eltFc4iLink.dataset.fc4iEntryLink = "true"; // FIX-ME:
        // jmDisplayed.update_node(node, html.outerHTML + eltFc4iLink.outerHTML);
        jmDisplayed.update_node(node, html.outerHTML);
...
function fixJmnodeProblem(eltJmnode) {
    // console.log("fixJmnodeProblem", eltJmnode);
    const customRenderer = getOurCustomRenderer();
    // FIX-ME: root
    ...
    const { eltTxt, eltBg } = addJmnodeBgAndText(eltJmnode);
...
function fixProblemsAndUpdateCustomAndShapes(jmDisplayed, eltJmnodes) {
    setTimeout(() => {
        console.log("fixJmnodesProblem (in setTimeout fun)");
        addDebugLog("fixJmnodesProblem (in setTimeout fun)");
        // [...document.getElementsByTagName("jmnode")].forEach(eltJmnode => 
        [...eltJmnodes.getElementsByTagName("jmnode")].forEach(eltJmnode => {
            if (isVeryOldCustomFormat(eltJmnode)) return;
            fixJmnodeProblem(eltJmnode); // FIX-ME: Remove when this is fixed in jsmind
...
function oldSecondJmnodesFixing() {
    return;
    setTimeout(() => {
        console.log("oldSecondJmnodesFixing, in timeout");
        addDebugLog("oldSecondJmnodesFixing (in setTimeout fun)");
        [...document.getElementsByTagName("jmnode")].forEach(eltJmnode => {
            const child1 = eltJmnode.firstElementChild;
            if (child1 && child1.tagName === "DIV") {
                // console.log("child1 && ...", eltJmnode);
                if (isVeryOldCustomFormat(eltJmnode)) return;
                fixOldCustomAndUpdate(eltJmnode);
                const node_id = jsMind.my_get_nodeID_from_DOM_element(eltJmnode);
                if (node_id == 21) console.warn("node_id 21");
...
function showKeyInFc4i(keyFc4i) {
    ...
    // objUrl.searchParams.set("showkey", "my-key")
